...
def _pick_least_loaded(servers: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for s in servers:
        cap, curr = int(s.get("capacity") or 0), int(s.get("current_users") or 0)
        try: curr = max(curr, db.count_services_on_node(s["name"]))
        except Exception: pass
        free = cap - curr if cap > 0 else 0
        if free > best_free: best, best_free = s, free
...
def _select_server(server_name: Optional[str] = None) -> Dict[str, Any]:
    ...
    if db_servers:
        ...
        if policy in ("least_loaded", "capacity", "free"): return _pick_least_loaded(db_servers)
        if policy == "by_name" and DEFAULT_SERVER_NAME:
            named = _db_get_server_by_name(DEFAULT_SERVER_NAME)
            if named and int(named.get("is_active", 1)) == 1: return named
        return db_servers[0]
