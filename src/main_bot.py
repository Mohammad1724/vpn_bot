...
async def check_expiring_services(context: ContextTypes.DEFAULT_TYPE):
    ...
    for service in expiring_services:
        try:
            # Assuming expiry_date in DB is a string like 'YYYY-MM-DD HH:MM:SS'
            expiry_date_obj = datetime.strptime(service['expiry_date'].split(' ')[0], '%Y-%m-%d')
            expiry_date_formatted = expiry_date_obj.strftime('%Y/%m/%d')
            await context.bot.send_message(
                chat_id=service['user_id'],
                text=f"📢 یادآوری: سرویس شما با لینک زیر در تاریخ **{expiry_date_formatted}** منقضی خواهد شد.\n\n`{service['sub_link']}`",
                parse_mode=ParseMode.MARKDOWN
            )
            await asyncio.sleep(0.1)
        except (Forbidden, BadRequest):
            logger.warning(f"Could not send expiry notification to user {service['user_id']}.")
        except Exception as e:
            logger.error(f"Error sending expiry notification for service {service.get('service_id')}: {e}")
...
async def get_custom_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(custom_name) > 50:
        await update.message.reply_text("نام وارد شده بیش از حد طولانی است. (حداکثر 50 کاراکتر)")
        return GET_CUSTOM_NAME
...
async def user_button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "showlinks":
        ...
        if success:
            db.update_balance(user_id, -plan['price'])
            db.renew_service(service_id, plan['days'])
...
async def admin_button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "admin" and data[1] == "confirm" and data[2] == "charge":
        ...
        if not user_message_sent:
            admin_feedback += "\n\n⚠️ **اخطار:** کاربر ربات را بلاک کرده یا آیدی نامعتبر است."
        ...
        if data[2] == "card" and data[3] == "number":
        ...
        elif data[2] == "card" and data[3] == "holder":
        ...
        if not restore_path or not os.path.exists(restore_path):
            await query.edit_message_text("خطا: فایل پشتیبان یافت نشد یا عملیات منقضی شده است.")
            return
        try:
...
async def send_service_details(context: ContextTypes.DEFAULT_TYPE, chat_id: int, service: dict, original_message=None):
    try:
        ...
        if info:
            ...
            if package_days > 0:
                try:
                    ...
                    logger.error(f"Could not parse start_date: {start_date_str} for UUID {service['sub_uuid']}")
                    is_expired = True
        ...
        logger.error(f"Error in send_service_details for service_id {service.get('service_id')}: {e}")
        error_text = f"❌ خطا در دریافت اطلاعات سرویس. لطفاً بعدا دوباره تلاش کنید یا با پشتیبانی تماس بگیرید."
        error_markup = InlineKeyboardMarkup([[InlineKeyboardButton("🔄 تلاش مجدد", callback_data=f"refresh_{service['service_id']}")]])
        try:
            if original_message:
                await original_message.edit_text(error_text, parse_mode=ParseMode.MARKDOWN, reply_markup=error_markup)
            else:
                await context.bot.send_message(chat_id=chat_id, text=error_text, parse_mode=ParseMode.MARKDOWN, reply_markup=error_markup)
        except Exception as inner_e:
            logger.error(f"Could not even send error message for service_id {service.get('service_id')}: {inner_e}")
...
async def broadcast_to_all_send(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for user_id_tuple in user_ids:
        try:
            await message_to_send.copy(chat_id=user_id_tuple[0])
            sent_count += 1
            await asyncio.sleep(0.1)
        except (Forbidden, BadRequest):
            failed_count += 1
...
async def user_management_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("لطفا آیدی عددی کاربری که می‌خواهید مدیریت کنید را وارد نمایید:", reply_markup=ReplyKeyboardMarkup([["بازگشت به منوی ادمین"]], resize_keyboard=True))
...
async def manage_user_amount_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        action, target_user_id = context.user_data['manage_action'], context.user_data['target_user_id']
...
async def backup_restore_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [["📥 دریافت فایل پشتیبان", "📤 بارگذاری فایل پشتیبان"], ["بازگشت به منوی ادمین"]]
...
async def restore_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("⚠️ **اخطار:** بازیابی دیتابیس تمام اطلاعات فعلی را پاک می‌کند.\nبرای ادامه، فایل دیتابیس (`.db`) خود را ارسال کنید. برای لغو `/cancel` را بزنید.", parse_mode=ParseMode.MARKDOWN)
...
def main():
    ...
    admin_conv = ConversationHandler(
        ...
        states={
            ADMIN_MENU: [
                ...
                MessageHandler(filters.Regex('^📊 آمار ربات$'), show_stats),
            ...
            BROADCAST_CONFIRM: [
                MessageHandler(filters.Regex('^بله، ارسال کن$'), broadcast_to_all_send),
                MessageHandler(filters.Regex('^خیر، لغو کن$'), admin_generic_cancel)
            ],
    ...
    application.add_handler(user_callbacks)
    application.add_handler(admin_callbacks)
