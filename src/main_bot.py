...
async def get_trial_service(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if result and result.get('uuid'): db.set_user_trial_used(user_id); db.add_active_service(user_id, result['uuid'], result['full_link'], 0, TRIAL_DAYS); await show_link_options_menu(msg_loading, result['uuid'], is_edit=True)
...
async def create_service_after_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if result and result.get('uuid'): db.update_balance(user_id, -plan['price']); db.add_active_service(user_id, result['uuid'], result['full_link'], plan['plan_id'], plan['days']); db.log_sale(user_id, plan['plan_id'], plan['price']); await show_link_options_menu(msg, result['uuid'], is_edit=True)
...
def main():
    ...
    print("Ultimate Bot is running with final fixes..."); application.run_polling()
