...
This file builds the PTB application, registers all handlers,
conversations, and error handlers.
"""
...
# Suppress a specific PTB warning
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if isinstance(update, Update):
        logger.error(f"Caused by update: {update}")


def build_application():
    """Builds and configures the PTB Application object."""
    ...
    gift_conv = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex(f'^{constants.BTN_GIFT_CODE}$') & user_filter, check_channel_membership(gift_h.gift_code_entry))],
        states={constants.REDEEM_GIFT: [MessageHandler(filters.TEXT & ~filters.COMMAND, gift_h.redeem_gift_code)]},
        fallbacks=[CommandHandler('cancel', gift_h.gift_cancel)],
        per_user=True, per_chat=True
    )
    ...
    charge_conv = ConversationHandler(
        ...
        fallbacks=[CommandHandler('cancel', charge_h.charge_cancel)],
        per_user=True, per_chat=True
    )
    ...
    support_conv = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex(f'^{constants.BTN_SUPPORT}$') & user_filter, check_channel_membership(support_h.support_ticket_start))],
    ...
    create_gift_conv = ConversationHandler(
        ...
        states={
            constants.CREATE_GIFT_AMOUNT: [MessageHandler(filters.TEXT & ~filters.COMMAND, admin_gift.create_gift_amount_received)]
        },
        fallbacks=[CommandHandler('cancel', admin_c.admin_generic_cancel)],
        map_to_parent={constants.ADMIN_MENU: constants.ADMIN_MENU}
    )
    ...
    broadcast_conv = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex(f'^{constants.BTN_BROADCAST}$') & admin_filter, admin_users.broadcast_menu)],
        states={
    ...
    admin_conv = ConversationHandler(
        ...
        states={
            constants.ADMIN_MENU: [
                MessageHandler(filters.Regex(f'^{constants.BTN_MANAGE_PLANS}$'), admin_plans.plan_management_menu),
                MessageHandler(filters.Regex(f'^{constants.BTN_REPORTS}$'), admin_reports.reports_menu),
                MessageHandler(filters.Regex(f'^{constants.BTN_SETTINGS}$'), admin_settings.settings_menu),
                MessageHandler(filters.Regex(f'^{constants.BTN_BACKUP}$'), admin_backup.backup_restore_menu),
                MessageHandler(filters.Regex(f'^{constants.BTN_MANAGE_USERS}$'), admin_users.user_management_menu),
                MessageHandler(filters.Regex(f'^{constants.BTN_SHUTDOWN}$'), admin_c.shutdown_bot),
                MessageHandler(filters.Regex(f'^{constants.BTN_MANAGE_GIFT_CODES}$'), admin_gift.gift_code_management_menu),
                MessageHandler(filters.Regex('^📋 لیست کدهای هدیه$'), admin_gift.list_gift_codes),
                MessageHandler(filters.Regex(f'^{constants.BTN_BACK_TO_ADMIN_MENU}$') & admin_filter, admin_c.back_to_admin_menu),
                create_gift_conv, settings_conv, broadcast_conv,
            ...
            constants.RESTORE_UPLOAD: [MessageHandler(filters.Document.ALL & admin_filter, admin_backup.restore_receive_file)],
            constants.MANAGE_USER_ID: [MessageHandler(filters.TEXT & ~filters.COMMAND, admin_users.manage_user_id_received)],
            constants.MANAGE_USER_ACTION: [MessageHandler(filters.TEXT & ~filters.COMMAND, admin_users.manage_user_action_handler)],
    ...
    # Main commands and menu buttons
    application.add_handler(CommandHandler("start", check_channel_membership(start_h.start)))
    application.add_handler(MessageHandler(filters.Regex(f'^{constants.BTN_BUY_SERVICE}$'), check_channel_membership(buy_h.buy_service_list)))
    application.add_handler(MessageHandler(filters.Regex(f'^{constants.BTN_MY_SERVICES}$'), check_channel_membership(us_h.list_my_services)))
    application.add_handler(MessageHandler(filters.Regex(f'^{constants.BTN_ACCOUNT_INFO}$'), check_channel_membership(start_h.show_account_info)))
    application.add_handler(MessageHandler(filters.Regex(f'^{constants.BTN_SUPPORT}$'), check_channel_membership(support_h.support_ticket_start)))
    application.add_handler(MessageHandler(filters.Regex(f'^{constants.BTN_GUIDE}$'), check_channel_membership(start_h.show_guide)))
    application.add_handler(MessageHandler(filters.Regex(f'^{constants.BTN_FREE_TRIAL}$'), check_channel_membership(trial_get_trial_service)))
    application.add_handler(MessageHandler(filters.Regex(f'^{constants.BTN_REFERRALS}$'), check_channel_membership(start_h.show_referral_link)))
