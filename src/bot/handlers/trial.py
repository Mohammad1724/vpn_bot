...
from config import (
    TRIAL_ENABLED, TRIAL_DAYS, TRIAL_GB, 
    PANEL_DOMAIN, ADMIN_PATH, API_KEY, SUBSCRIPTION_LINK
)
...
async def get_trial_service(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        new_uuid = hiddify.add_user(name, TRIAL_GB, TRIAL_DAYS, note)
        if not new_uuid:
            raise RuntimeError("Provisioning for trial failed or no uuid returned.")
        db.add_active_service(user_id, name, new_uuid, plan_id=None)
        ...
        if user_data:
            ...
            await update.message.reply_text(
                "✅ سرویس تست شما با موفقیت ساخته شد، اما در دریافت اطلاعات سرویس مشکلی پیش آمد. لطفاً از منوی «سرویس‌های من» آن را مشاهده کنید.",
                reply_markup=get_main_keyboard(user_id)
            )
