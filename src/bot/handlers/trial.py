...
async def get_trial_service(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # <<< FIX: Logic is now handled here instead of keyboards.py
    if not TRIAL_ENABLED:
        await update.message.reply_text("سرویس تست رایگان در حال حاضر غیرفعال است.")
        return
        
    if user_info.get('has_used_trial'):
        await update.message.reply_text("شما قبلاً از سرویس تست رایگان استفاده کرده‌اید و دیگر امکان دریافت آن وجود ندارد.")
        return
    ...
    if result and result.get('uuid'):
        ...
        # Pass context to avoid NameError
