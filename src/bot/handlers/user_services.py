...
async def send_service_details(
    ...
    if not service:
        ...
        # FIX: Call edit_text correctly without chat_id
        if original_message:
        ...
        return
    try:
        info = await hiddify_api.get_user_info(service['sub_uuid'])  # Removed server_name for simplicity
...
async def show_link_options_menu(message: Message, user_uuid: str, service_id: int, is_edit: bool = True, context: ContextTypes.DEFAULT_TYPE = None):
    buttons = [
        ...
        btn("Clash Meta", f"getlink_clashmeta_{user_uuid}"), btn("📄 کانفیگ‌های تکی", f"getlink_full_{user_uuid}"),
    ]
