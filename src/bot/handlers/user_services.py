...
async def send_service_details(context: ContextTypes.DEFAULT_TYPE, chat_id: int, service_id: int, original_message=None, is_from_menu: bool = False):
    ...
    try:
        ...
        if not info:
            ...
            text = "❌ اطلاعات این سرویس در پنل یافت نشد. احتمالاً حذف یا منقضی شده است.\nمی‌خواهید این سرویس از ربات هم حذف شود؟"
            if original_message:
...
async def show_link_options_menu(message: Message, user_uuid: str, is_edit: bool = True):
    keyboard = [
    ...
    text = "سرویس شما ساخته شد. لطفاً نوع لینک اشتراک مورد نظر را انتخاب کنید:"
    try:
        if is_edit:
...
async def get_link_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    _, link_type, user_uuid = q.data.split('_')
    await q.message.edit_text("در حال ساخت لینک و QR Code... ⏳")
    sub_path = SUB_PATH or ADMIN_PATH
    sub_domain = random.choice(SUB_DOMAINS) if SUB_DOMAINS else PANEL_DOMAIN
    ...
    img = qrcode.make(final_link_with_fragment)
    bio = io.BytesIO(); bio.name = 'qrcode.png'; img.save(bio, 'PNG'); bio.seek(0)
    caption = (
        f"نام کانفیگ: **{config_name}**\n\n"
        f"با اسکن QR یا استفاده از لینک زیر متصل شوید:\n\n"
        f"`{final_link_with_fragment}`"
    )
    await q.message.delete()
    await context.bot.send_photo(
