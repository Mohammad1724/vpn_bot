...
async def admin_view_service(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not svc:
        ...
        return
    user_data = hiddify.get_user(svc['sub_uuid'])
    if user_data:
...
async def admin_renew_service(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    res = hiddify.renew_user(svc['sub_uuid'], plan['gb'], plan['days'])
...
async def admin_delete_service(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    success = hiddify.delete_user(svc['sub_uuid'])
    if success:
...
async def broadcast_to_all_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data.clear(); context.user_data["broadcast_mode"] = "all"
    await update.message.reply_text("متن/رسانه پیام همگانی را بفرستید:", reply_markup=ReplyKeyboardMarkup([[BTN_BACK_TO_ADMIN_MENU]], resize_keyboard=True))
...
async def broadcast_to_all_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"پیش‌نمایش ثبت شد. ارسال به {len(total_users)} کاربر انجام شود؟", reply_markup=keyboard)
...
async def broadcast_to_user_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("شناسه عددی کاربر را بفرستید:", reply_markup=ReplyKeyboardMarkup([[BTN_BACK_TO_ADMIN_MENU]], resize_keyboard=True))
...
async def broadcast_to_user_id_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try: uid = int(update.message.text.strip()); assert uid > 0
        ...
        await update.message.reply_text("شناسه معتبر نیست. یک عدد مثبت بفرستید."); return BROADCAST_TO_USER_ID
...
async def broadcast_to_user_message_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not uid:
        await update.message.reply_text("شناسه کاربر مشخص نیست."); context.user_data.clear(); return ConversationHandler.END
    msg = update.message
    try:
        await context.bot.copy_message(chat_id=uid, from_chat_id=msg.chat.id, message_id=msg.message_id)
