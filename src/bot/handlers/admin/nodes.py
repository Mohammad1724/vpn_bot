...
def _build_node_settings_view() -> Tuple[str, InlineKeyboardMarkup]:
    ...
    text = (
        ...
        f"▫️ سیاست انتخاب نود: {h(policy)}\n\n"
        f"▫️ Health‑check نودها: {h(nodes_h_on)}"
    )
    keyboard = [
...
async def edit_node_setting_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if key in ("nodes_health_interval_min", "nodes_auto_disable_after_fails"):
        ...
        tip = "\n(یکی از: <code>first</code>, <code>by_name</code>, <code>least_loaded</code>)"

    text = (
        f"✍️ مقدار جدید برای <b>{h(key)}</b> را ارسال کنید.{tip}\n"
...
async def add_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    if q := update.callback_query:
        await q.answer()
        await q.edit_message_text("عملیات لغو شد.", reply_markup=_nodes_menu_kb())
    return ConversationHandler.END
...
async def list_nodes(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not nodes:
        await update.callback_query.edit_message_text(
            "هیچ نودی ثبت نشده.",
            reply_markup=markup([nav_row(back_cb="admin_nodes", home_cb="home_menu")])
        )
        return C.NODES_MENU
...
async def show_node_usage(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    total_gb, users_cnt = _sum_server_usage(n["name"])
    text = (
        f"📊 مصرف snapshot «{n['name']}»:\n"
        f"- مجموع مصرف: {total_gb:.2f} GB\n"
        f"- برای {users_cnt} کاربر\n\n"
        f"این مقادیر با جاب زمان‌بندی به‌روز می‌شود."
    )
