...
async def add_get_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not name:
        await update.message.reply_text("نام معتبر نیست."); return ADD_NAME
...
async def add_get_panel_domain(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not panel_domain:
        await update.message.reply_text("دامنه معتبر نیست."); return ADD_PANEL_DOMAIN
...
async def add_get_admin_path(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not admin_path:
        await update.message.reply_text("مقدار معتبر نیست."); return ADD_ADMIN_PATH
...
async def add_get_sub_path(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not sub_path:
        await update.message.reply_text("مقدار معتبر نیست."); return ADD_SUB_PATH
...
async def add_get_api_key(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not api_key:
        await update.message.reply_text("API Key خالی است."); return ADD_API_KEY
...
async def add_get_capacity(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    txt = (update.message.text or "").strip()
    if not txt.isdigit():
        await update.message.reply_text("عدد معتبر وارد کنید."); return ADD_CAPACITY
...
async def edit_field_value_received(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if field == "capacity":
        if not value.isdigit():
            await update.message.reply_text("عدد معتبر وارد کنید."); return EDIT_FIELD_VALUE
        value = int(value)
    elif field in ("admin_path", "sub_path"):
        value = value.strip().strip("/")
        if not value:
            await update.message.reply_text("مقدار معتبر نیست."); return EDIT_FIELD_VALUE
