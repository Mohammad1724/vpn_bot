...
def get_admin_keyboard():
    ...
    markup.row(KeyboardButton("➕ افزودن پلن"), KeyboardButton("📋 لیست/حذف پلن‌ها"))
...
def handle_admin_panel(message):
    ...
    if message.text == "➕ افزودن پلن":
    ...
    elif message.text == "📋 لیست/حذف پلن‌ها":
        ...
        bot.send_message(ADMIN_ID, "این بخش در حال توسعه است.")
    else:
        bot.send_message(ADMIN_ID, "دستور نامشخص است. لطفاً از دکمه‌ها استفاده کنید.", reply_markup=get_admin_keyboard())
...
def show_plan_management_panel(chat_id):
    ...
    for plan in plans:
        ...
        markup.add(InlineKeyboardButton(f"🗑 حذف پلن: {plan['name']}", callback_data=f"delete_plan_{plan['plan_id']}"))
...
def process_charge_user_amount(message): # نام تابع برای جلوگیری از تداخل
    try:
