...
def update_user_balance(user_id, amount, top_up=True):
    conn, c = db_connect()
    # اصلاح کلیدی: ابتدا مطمئن می‌شویم کاربر در دیتابیس وجود دارد.
    ...
    if top_up:
        ...
        logger.info(f"موجودی کاربر {user_id} به مقدار {amount} افزایش یافت.")
    else:
        c.execute("UPDATE users SET wallet_balance = wallet_balance - ? WHERE user_id = ?", (amount, user_id))
        logger.info(f"موجودی کاربر {user_id} به مقدار {amount} کاهش یافت.")
    conn.commit()
    conn.close()
...
def handle_receipt(message):
    user = message.from_user
    add_or_update_user(user.id, user.first_name, user.username) # اطمینان از وجود کاربر
    ...
    try:
        ...
        bot.reply_to(message, "خطایی در ثبت مبلغ درخواستی شما رخ داده. لطفاً دوباره تلاش کنید.")
    ...
    markup = InlineKeyboardMarkup()
    callback_data = f"approve_charge_{user.id}_{requested_amount}"
...
def backup_data(chat_id):
    # (پیاده‌سازی این تابع در صورت نیاز)
    pass
    ...
    if user_id == ADMIN_ID:
        ...
        if data == "add_plan":
            ...
            try:
                ...
                logger.info(f"ادمین کیف پول کاربر {target_user_id} را به مبلغ {amount_to_add} شارژ کرد.")
            except (IndexError, ValueError) as e:
                logger.error(f"خطا در پردازش callback تایید شارژ: {e} | Data: {data}")
                bot.edit_message_text("خطا در پردازش اطلاعات. لطفاً مطمئن شوید مبلغ صحیح است.", call.message.chat.id, call.message.message_id)
...
def handle_admin_state_messages(message):
    ...
    if state == "adding_plan_name":
        ...
        except ValueError: bot.send_message(chat_id, "خطا: لطفاً قیمت را به صورت عدد وارد کنید.")
        ...
        except ValueError: bot.send_message(chat_id, "خطا: لطفاً مدت زمان را به صورت عدد وارد کنید.")
