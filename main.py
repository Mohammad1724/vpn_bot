# main.py (نسخه نهایی، قطعی و پایدار)
...
PROMPTS = {
    "adding_plan_name": "🔹 ۱/۴: نام پلن:",
    "adding_plan_price": "🔹 ۲/۴: قیمت (فقط عدد):",
    "adding_plan_duration": "🔹 ۳/۴: زمان (روز):",
...
# (توابع دیتابیس و کمکی بدون تغییر)
def db_connect():
...
def handle_admin_panel(message):
    ...
    elif message.text == "📥 بکاپ": backup_data(chat_id)
...
def show_statistics(chat_id):
    ...
    stats_text = (
        ...
        f"   -  مشتریان فعال: {active_customers}\n\n"
        f"📈 **فروش و درآمد:**\n"
        f"   -  تعداد کل فروش‌ها: {total_sales}\n"
        f"   -  درآمد کل: {total_income:,} تومان\n\n"
        f"📋 **پلن‌ها:**\n"
        f"   -  تعداد پلن‌های فعال: {total_plans}"
    )
